...
export const useUserStore = defineStore('user', () => {
  ...
  const user = ref<User>({
    ...
    blocked: JSON.parse(localStorage.getItem('blocked') || 'false'), 
    confirmed: JSON.parse(localStorage.getItem('confirmed') || 'false'), 
    role: {
  ...
  return { user, username, jwt, role, isAuthenticated, authenticaded, logout,isAdmin}
})